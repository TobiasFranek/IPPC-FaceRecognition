...
export default ({ faces, onFacesChange }) => {
  ...
  const remember = () => {
    facesAPI.rememberFace(open, name).then(() => {
      handleClose();
      if (onFacesChange) {
        onFacesChange(faces.filter((face) => face.id !== open));
      }
    });
  }
  ...
  return (
    <>
            ...
            {faces.map((face) => (
              <TableRow key={face.id}>
                ...
                <TableCell align="center">
                  <List component="nav">
                    <ListItem button onClick={() => handleClickOpen(face.id)}>
                      <ListItemIcon>
                        <CreateIcon />
                      </ListItemIcon>
      ...
      <Dialog open={open !== null ? true : false} onClose={handleClose} aria-labelledby="form-dialog-title">
        <DialogTitle id="form-dialog-title">Remember</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
        ...
        <DialogActions>
          ...
          <Button onClick={remember} color="primary">
            Remember
          </Button>
        </DialogActions>
      </Dialog>
